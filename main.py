...
def p_statement_ifcearacati(p):
    '''
    statement : IFCEARACATI OP_FINALLINHA
    '''
...
class Application():
    ...
    def chama_analisador(self):
        ...
        for tok in lexer:
            ...
            if tok.type == "string_mf":
                ...
                add_lista_saida(tok, f" ")
        if (saidas[0][3] == "IFCEARACATI"):
            if (saidas[1][3] != "'"):
                erros += 1
                self.saida.insert('', tk.END, values="Algoritmo sem IFCEARACATI no início, condicao obrigatoria")
            else:
                pass
        else:
            erros += 1
    ...
    def Menus(self):
        ...
        def tokens():
            ...
            newWindow.insert("", 1, text="", values=("ifcearacati", "ifcearacati", "ifcearacati", "Palavra Reservada ifcearacati"))
